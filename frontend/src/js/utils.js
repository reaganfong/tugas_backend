...
function checkAuth(requiredRole = null, redirectUrl = '/views/index.html') {
    ...
    console.log('[CHECK AUTH] window.location:', window.location.pathname);

    if (!jabatan || !username) {
        console.log('[CHECK AUTH] ⚠️ localStorage kosong!');
        
        // Coba ambil dari URL params (untuk testing)
        const urlParams = new URLSearchParams(window.location.search);
        const urlJabatan = urlParams.get('jabatan');
        const urlUsername = urlParams.get('username');
        
        if (urlJabatan && urlUsername) {
            console.log('[CHECK AUTH] 🔄 Menggunakan data dari URL params');
            localStorage.setItem('jabatan', urlJabatan);
